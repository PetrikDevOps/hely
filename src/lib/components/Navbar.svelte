    ...
    <div class="flex items-center gap-4">
      ...
      <ul class="hidden md:flex items-center gap-4">
        ...
        {#if item.auth && data.session || !item.auth}
          <li>
            <button onclick={item.navFn} class="flex items-center gap-1 hover:bg-gray-200 hover:text-black rounded-lg p-2 transition-colors">
              <Icon icon={item.icon} />
              <span>{item.name}</span>
            </button>
          </li>
    ...
    <div class="hidden items-center gap-4 md:flex md:w-auto">
      ...
      {:else}
        <button 
          ...
          class="flex items-center gap-1 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 hover:bg-opacity-50 bg-opacity-20 transition-colors"
        >
      ...
      <ul class="absolute flex w-full flex-col gap-6 bg-black bg-opacity-80 p-4 py-4 font-semibold">
        ...
        {#if !data.session}
          <li>
            <button 
              onclick={() => signIn('microsoft-entra-id')} 
              class="flex items-center gap-1 hover:bg-gray-200 hover:text-black rounded-lg p-2 transition-colors"
            >
              <Icon icon="mdi:login" /> 
              <span>Bejelentkezés</span>
            </button>
