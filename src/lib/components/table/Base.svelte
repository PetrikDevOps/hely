...
<div class="overflow-hidden rounded-xl shadow-xl">
  <div
    class="flex items-center justify-between border-b-2 border-petrik-3 bg-black bg-opacity-30 px-2 md:px-4 py-2"
  >
    <div class="flex gap-0.5 md:gap-2">
      <h1 class="font-bold md:text-2xl">{title}</h1>
      <p class="font-light hidden self-end md:block md:text-lg">{data.length} db</p>
    </div>
    <h2 class="hidden md:block text-2xl font-semibold">{dayjs(date).format("LL")} - {dayjs(date).format("dddd")}</h2>
    ...
    <thead>
      <tr class="border-b border-petrik-3 bg-black bg-opacity-20">
        {#each tableHeaders as header}
          <th class="px-2 md:px-4 md:py-2 py-1 text-left font-semibold" class:hide-mobile={header.hideMobile === true}>
